...
mongoose.connect('mongodb+srv://nysa:12345@cluster0-cz3ei.mongodb.net/blogDB', {
...
app.get('/', (req, res) => {
  Post.find({}, (err, allPosts) => {
    if (!err) 
      res.render('home', {
        eHomeContent: homeStartingContent, 
        ePosts: allPosts
      });
  })
});
...
app.get('/posts/:id', (req, res) => {
  Post.findOne({_id: req.params.id}, (err, onePost) => {
    if (!err)
      res.render('post', {ePostTitle: onePost.title, ePostPost: onePost.post})
  });
  /*posts.forEach(e => {
    if(_.lowerCase(e.title) === _.lowerCase(req.params.title))
